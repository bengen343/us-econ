"""Cloud Run Job entrypoint for the Michigan sentiment forecasts.

Runs daily: the pending release alternates between the next month's
preliminary and the current month's final, so every run re-nowcasts exactly
one target as its high-frequency inputs (gasoline spot, S&P 500) accrue.
There is no calendar gate -- the pending-target logic in the shared model
self-gates, and rows are upserted by as_of_date (idempotent on same-day
retry; a new row per day preserves the revision trajectory). The _current
view surfaces the latest generation per (target, target_month,
model_version).
...
from forecasts.michigan_sentiment.headline.production import models
...
def _ensure_table_and_view(client: bigquery.Client) -> None:
    ...
    OPTIONS (description = 'Michigan ICS forecasts -- the pending preliminary '
      '(level + change vs prior final) or pending final (level + revision vs '
      'the preliminary) -- from survey-window gasoline + S&P OLS; append-only '
      'pre-release revisions, one row per (target, target_month, as_of_date, '
      'model_version).')
    """).result()
...
def main() -> None:
    run_id = _run_id()
    log = configure_logging("forecast.michigan_sentiment", run_id)
    ...
    if not forecasts:
        log.warning("no pending target computable; skipping")
        return
