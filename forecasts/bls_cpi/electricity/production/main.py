"""Cloud Run Job entrypoint for the electricity-price forecast.

Runs daily through the pre-release window (the first ~half of the month, before
the CPI/average-price print for the prior month drops ~the 10th-15th). Flow:

  1. Pull the electricity average price from BigQuery (read-only) and compute
     the next-month nowcast (level $/kWh and m/m %).
...
from forecasts.bls_cpi.electricity.production import models

FORECAST_WINDOW_LAST_DAY = 18  # the AP print releases with CPI, ~the 10th-15th


def _in_forecast_window(d: date) -> bool:
    """First ~18 days of the month — the run-up to (and a buffer past) the
...
def _ensure_table_and_view(client: bigquery.Client) -> None:
    ...
    OPTIONS (description = 'Electricity average-price forecasts ($/kWh level and '
      'm/m %) from the own-history seasonal AR OLS; append-only pre-release '
      'revisions, one row per (target, target_month, as_of_date, model_version).')
    """).result()
...
def main() -> None:
    run_id = _run_id()
    log = configure_logging("forecast.electricity", run_id)
