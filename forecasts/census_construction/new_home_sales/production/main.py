"""Cloud Run Job entrypoint for the new-home-sales forecast.

Runs daily: the forecastable window opens when the target month's SF permits
land with the NRC release (~the 17th of M+1) and closes at the New
Residential Sales release (~the 23rd-27th, 10:00 ET); outside it the shared
model returns None and the job idles. Flow:

  1. Pull inputs from BigQuery (read-only) and compute the next-month sales
     nowcast (SAAR level and m/m %).
...
from forecasts.census_construction.new_home_sales.production import models
...
def _ensure_table_and_view(client: bigquery.Client) -> None:
    ...
    OPTIONS (description = 'New home sales forecasts (SAAR level + m/m %) for '
      'the next New Residential Sales release -- from the same-month '
      'SF-permits OLS (the sales sample is permit-drawn); append-only '
...
def main() -> None:
    run_id = _run_id()
    log = configure_logging("forecast.new_home_sales", run_id)
    ...
    if not forecasts:
        log.warning("no forecast computable (month-M SF permits not yet published); skipping")
        return
