"""Cloud Run Job entrypoint for the CPI-gasoline forecast.

Runs daily through the pre-release window (the first ~half of the month, before
the CPI print for the prior month drops ~the 10th-15th). Flow:

  1. Pull inputs from BigQuery (read-only) and compute the next-month CPI
     gasoline nowcast (SA index level and m/m %).
...
from forecasts.bls_cpi.gasoline.production import models
...
def _ensure_table_and_view(client: bigquery.Client) -> None:
    ...
    OPTIONS (description = 'CPI gasoline (all types, SA) forecasts -- index level and '
      'm/m % -- from the retail pass-through + seasonal-wedge OLS; append-only '
      'pre-release revisions, one row per (target, target_month, as_of_date, model_version).')
    """).result()
...
def main() -> None:
    run_id = _run_id()
    log = configure_logging("forecast.gasoline", run_id)
