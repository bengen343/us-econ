...
from collectors.bls_ppi.series import PPI_SERIES, PpiSeries
...
def collect(settings: Settings) -> LoadSpec:
    ...
    series_index = {s.series_id: s for s in PPI_SERIES}
...
def _rows_for_series(meta: PpiSeries, points: list[dict]) -> list[dict]:
    rows: list[dict] = []
    for point in points:
