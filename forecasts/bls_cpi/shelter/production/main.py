"""Cloud Run Job entrypoint for the CPI-shelter forecast.
...
the CPI print for the prior month drops ~the 10th-15th). Flow:
  ...
  1. Pull the shelter SA index from BigQuery (read-only) and compute the
     next-month nowcast (SA index level and m/m %).
...
from forecasts.bls_cpi.shelter.production import models
...
def _ensure_table_and_view(client: bigquery.Client) -> None:
    ...
    OPTIONS (description = 'CPI shelter (SA) forecasts -- index level and m/m % -- '
      'from the trailing-6-month-mean nowcast; append-only pre-release revisions, '
...
def main() -> None:
    run_id = _run_id()
    log = configure_logging("forecast.shelter", run_id)
