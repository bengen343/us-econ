"""Cloud Run Job entrypoint for the ISM Manufacturing PMI forecast.

Runs daily: the forecastable window opens when the month-M S&P flash lands
(~the 21st-24th) and closes at the ISM release (1st business day of M+1,
10:00 ET); outside it the shared model returns None and the job idles. Flow:

  1. Pull inputs from BigQuery (read-only) and compute the next-month PMI
  ...
  3. Upsert this run's rows keyed by as_of_date (idempotent on same-day
     retry; a new row per day preserves the revision trajectory as late Fed
     surveys arrive). The _current view surfaces the latest generation per
     (target, target_month, model_version).
...
from forecasts.ism.manufacturing_pmi.production import models
...
def _ensure_table_and_view(client: bigquery.Client) -> None:
    ...
    OPTIONS (description = 'ISM Manufacturing PMI forecasts -- level and m/m '
      'change -- from the S&P-flash + regional-Fed-composite OLS; append-only '
...
def main() -> None:
    run_id = _run_id()
    log = configure_logging("forecast.ism_mfg", run_id)
    ...
    if not forecasts:
        log.warning("no forecast computable (month-M flash not yet published); skipping")
        return
