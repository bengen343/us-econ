"""Cloud Run Job entrypoint for the egg-price forecast.
...
the CPI/average-price print for the prior month drops ~the 10th-15th). Flow:
  ...
  1. Pull inputs from BigQuery (read-only) and compute the next-month egg-price
     nowcast (level $/dozen and m/m %).
...
from forecasts.bls_cpi.eggs.production import models
...
def _ensure_table_and_view(client: bigquery.Client) -> None:
    ...
    OPTIONS (description = 'Egg-price forecasts (BLS average price, $/dozen level '
      'and m/m %) from the AR + PPI-eggs distributed lag + seasonal OLS; append-only '
...
def main() -> None:
    run_id = _run_id()
    log = configure_logging("forecast.eggs", run_id)
