"""Cloud Run Job entrypoint for the core-PCE m/m forecast.

Runs daily: the forecastable window opens when the target month's CPI and
PPI prints land (mid-M+1) and closes at the PCE release (~25th-31st of
M+1); outside it the shared model returns None and the job idles. Flow:

  1. Pull inputs from BigQuery (read-only) and compute the next-month core
     PCE nowcast (m/m % and index level).
...
from forecasts.bea_pce.core_mm.production import models
...
def _ensure_table_and_view(client: bigquery.Client) -> None:
    ...
    OPTIONS (description = 'Core PCE (DPCCRG) forecasts -- m/m % and index '
      'level -- from the CPI/PPI translation OLS (month-M core CPI + PPI '
      'airfares); append-only pre-release revisions, one row per (target, '
      'target_month, as_of_date, model_version).')
    """).result()
...
def main() -> None:
    run_id = _run_id()
    log = configure_logging("forecast.core_pce", run_id)
    ...
    if not forecasts:
        log.warning("no forecast computable (target-month CPI/PPI not yet published); skipping")
        return
