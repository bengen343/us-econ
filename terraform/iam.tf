...
resource "google_bigquery_dataset_iam_member" "runner_noaa_climate_editor" {
  dataset_id = google_bigquery_dataset.noaa_climate.dataset_id
  role       = "roles/bigquery.dataEditor"
  member     = "serviceAccount:${google_service_account.runner.email}"
}

resource "google_bigquery_dataset_iam_member" "runner_bea_vehicles_editor" {
  dataset_id = google_bigquery_dataset.bea_vehicles.dataset_id
  role       = "roles/bigquery.dataEditor"
  member     = "serviceAccount:${google_service_account.runner.email}"
}

resource "google_bigquery_dataset_iam_member" "runner_bea_pce_editor" {
  dataset_id = google_bigquery_dataset.bea_pce.dataset_id
  role       = "roles/bigquery.dataEditor"
  member     = "serviceAccount:${google_service_account.runner.email}"
}
