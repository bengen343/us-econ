"""Cloud Run Job entrypoint for the airline-fares forecast.
...
the CPI print for the prior month drops ~the 10th-15th). Flow:
  ...
  1. Pull inputs from BigQuery (read-only) and compute the next-month CPI
     airline-fares nowcast (SA index level and m/m %).
...
from forecasts.bls_cpi.airfares.production import models
...
def _ensure_table_and_view(client: bigquery.Client) -> None:
    ...
    OPTIONS (description = 'CPI airline fares (SA) forecasts -- index level and '
      'm/m % -- from the AR(2) + WTI distributed-lag OLS; append-only pre-release '
...
def main() -> None:
    run_id = _run_id()
    log = configure_logging("forecast.airfares", run_id)
