"""Cloud Run Job entrypoint for the headline-PPI y/y forecast.

Runs daily through the pre-release window (the first ~half of the month,
before the PPI print for the prior month drops ~the 11th-16th). Flow:

  1. Pull inputs from BigQuery (read-only) and compute the next-month headline
     PPI final demand nowcast (y/y %, m/m %, NSA index level).
...
from forecasts.bls_ppi.headline_yy.production import models

FORECAST_WINDOW_LAST_DAY = 18  # PPI for the prior month releases ~the 11th-16th
...
def _ensure_table_and_view(client: bigquery.Client) -> None:
    ...
    OPTIONS (description = 'Headline PPI final demand (WPUFD4, NSA) forecasts -- '
      'y/y %, m/m %, and index level -- from the pricing-date energy + ISM '
      'prices-paid OLS; append-only pre-release revisions, one row per '
...
def main() -> None:
    run_id = _run_id()
    log = configure_logging("forecast.ppi_headline", run_id)
