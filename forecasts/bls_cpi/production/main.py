"""Cloud Run Job entrypoint for the CPI forecast.

Runs daily through the pre-release window (the first ~half of the month, before
CPI for the prior month drops ~the 10th-15th) so the deterministic nowcast for
the about-to-be-released month firms up as that month's fuel prices finalise.
Flow:

  1. Pull inputs from BigQuery (read-only) and compute the DMS nowcast for the
     next-to-be-released month (headline/core, m/m and y/y).
  2. Ensure the output table + _current view exist.
  3. Upsert this run's rows keyed by as_of_date (idempotent on same-day retry; a
     new row per day preserves the revision trajectory). The _current view
...
from forecasts.bls_cpi.production import models

FORECAST_WINDOW_LAST_DAY = 18  # CPI for the prior month releases ~the 10th-15th


def _in_forecast_window(d: date) -> bool:
    """First ~18 days of the month — the run-up to (and a buffer past) the CPI
    release. The job re-nowcasts the next-to-be-released month each day."""
...
def _ensure_table_and_view(client: bigquery.Client) -> None:
    ...
    OPTIONS (description = 'CPI forecasts (headline & core, m/m SA and y/y) from the '
      'deterministic bottom-up reconstruction; append-only pre-release revisions, '
...
def main() -> None:
    run_id = _run_id()
    log = configure_logging("forecast.cpi", run_id)
    ...
    if not forecasts:
        log.warning("no forecast produced (target month inputs incomplete); skipping")
        return
